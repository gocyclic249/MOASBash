...
if [ ${#MISSING_TOOLS[@]} -gt 0 ]; then
    ...
    if [ "$HAS_SS" = false ] && [ "$HAS_NETSTAT" = false ]; then
        ...
        echo ""
        echo "    Package names for ss (preferred):"
        echo "      Debian/Ubuntu: iproute2"
        echo "      CentOS/RHEL:   iproute"
        echo ""
        echo "    Package names for netstat (alternative):"
        echo "      Debian/Ubuntu: net-tools"
        echo "      CentOS/RHEL:   net-tools"
        echo ""
        echo "    Offline download links:"
        echo "      Debian:  https://packages.debian.org/search?keywords=iproute2"
        echo "      Ubuntu:  https://packages.ubuntu.com/search?keywords=iproute2"
        echo "      CentOS:  https://vault.centos.org/ (search for iproute)"
        echo "      RHEL:    https://access.redhat.com/downloads/content/package-browser"
    fi
    if [ "$HAS_JQ" = false ]; then
        echo "    - Log parsing will use basic mode (less detailed)"
        echo ""
        echo "    Package name: jq (same for all distros)"
        echo ""
        echo "    Offline download links:"
        echo "      Debian:  https://packages.debian.org/search?keywords=jq"
        echo "      Ubuntu:  https://packages.ubuntu.com/search?keywords=jq"
        echo "      CentOS:  https://vault.centos.org/ (search for jq)"
        echo "      RHEL:    https://access.redhat.com/downloads/content/package-browser"
        echo "      Static:  https://github.com/jqlang/jq/releases (portable binary)"
    fi
    if [ "$HAS_UNZIP" = false ]; then
        echo "    - SCC bundle extraction will not work"
        echo ""
        echo "    Package name: unzip (same for all distros)"
        echo ""
        echo "    Offline download links:"
        echo "      Debian:  https://packages.debian.org/search?keywords=unzip"
        echo "      Ubuntu:  https://packages.ubuntu.com/search?keywords=unzip"
        echo "      CentOS:  https://vault.centos.org/ (search for unzip)"
        echo "      RHEL:    https://access.redhat.com/downloads/content/package-browser"
    fi
    echo ""
fi
